  ...
  <v-card class="mx-auto" width="850">
    <v-card-title class="text-center pa-0">
      <v-toolbar flat dark color="#2466E0" class="text-white">
        <v-toolbar-title>Taxa</v-toolbar-title>
      </v-toolbar>
    </v-card-title>
    <v-card-text class="mt-5">
      <div class="d-flex align-center justify-space-between mb-6">
        <v-card
          ...
          width="100%"
          v-for="item in controller.modelTaxa.value"
          :key="item"
          :title="item.nome"
          :text="item.valor"
        >
      ...
      <v-row dense>
        <v-col cols="12">
          <v-text-field
            color="#2466E0"
            label="Taxa"
            placeholder="Digite aqui a sigla da taxa"
            variant="underlined"
            density="compact"
            clearable
            v-model="controller.taxa.value"
          ></v-text-field>
        </v-col>
      </v-row>
    </v-card-text>
    ...
    <v-card-actions>
      <v-btn
        ...
        @click="controller.getInformationTaxa()"
      >
        BUSCAR
      </v-btn>
    </v-card-actions>
